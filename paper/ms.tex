...
\begin{abstract}
\gb{Key messages are: WHY do users need to create their own products? (RB, asteroids, variable apertures, crowding etcetc). WHAT are we providing? (lightkurve, easy Kepler/K2/TESS access). HOW can they use it? (Beautiful examples)}
  \begin{itemize}
  \item{Performing custom photometry allows users to mitigate...}
  \item{However, with complex `big' data from \kepler and \ktwo...}
  \item{We present \lightkurve, an open source package for analysis of flux time series using Python.}
  \item{It is a multipurpose, general tool to make producing time-series photometry from raw pixel data easier and more reproducible between different teams.}
  \item{\lightkurve~ is designed to interface seemlessly with data from NASA's \kepler, \ktwo~ and \tess~ missions.}
  \item{Using these tools it is simple to create corrected time-series photometry from raw pixel data from any of these missions.}
  \item{Easy to interact with raw data products and produce fantastic light curves}
  \item{Open source, modable}
  \item{Supremely reproducible.}
  \item{Go from raw data to planet in 5 lines of code.}
  \item{Excellent learning tool}
  \end{itemize}

\clh{
What plots do we want? These should be our KILLER selling points. All graphs should be easy to understand as standalone figures. These are what will grab people! But let's keep it short and sweet.
  \begin{itemize}
...
%CLH: This might be unnecessary
\subsection{Light curves from large images}

\section{Example Code Snippets}

\subsection{Quickly recover a planet}
\begin{figure}
...
\subsection{Correct a supernova light curve}

\subsection{Background Subtraction for AGN}
...
%CLH: I don't think we need this figure
