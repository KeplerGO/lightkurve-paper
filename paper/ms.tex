...
\subsection{The \LightCurve~and \KeplerLightCurve~class}
        ...
        A \LightCurve
        object can be instantiated by passing a \texttt{time} array, a
        \texttt{flux} array, and, optionally, a \texttt{flux\_err} array
...
expressed as
\begin{align}
    \bm{\theta}^{*} = \argmin_{\bm{\theta} \in \Theta} \mathcal{C}(\bm{\theta}, f_{SAP}).
\end{align}
...
as
\begin{align}
    \bm{\theta}^{*}(\bm{y}) = \argmin_{\bm{\theta} \in \Lambda} \sum_{i=1}^{n}
...
motion-dependent artifacts~\cite{vanderburg14}.
